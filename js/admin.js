...
async function checkRemoteOnLoad(){
  ...
  try{
    ...
    if(remoteIsNewer){
      if(DATA.activeTimer){
        // فيه عداد شغال حالياً بهذا الجهاز — ما نبدّل البيانات تحته حتى ما نخرب الجلسة، بس بعد ما يخلّص العداد راح تنعرض النسخة الأحدث بفتحة الصفحة الجاية
        console.log('checkRemoteOnLoad: skipped auto-adopt, local timer running');
      } else {
        // نتبنّى تلقائياً بدون أي تدخل — هذا يخلي الموقع "نسخة وحدة" فعلياً بين كل أجهزتك، بلا زر تحميل يدوي
        DATA = mergeWithDefaults(remote);
        saveData(DATA); // نخزنها محلياً بهذا الجهاز أيضاً (بلا إعادة دفعها للسحابة، مالها داعي، هي أصلاً وحدة).
        applyTheme(DATA.settings);
        renderAll();
        renderBrandName();
        renderGoalTiersInputs();
        toast('محدّث لآخر نسخة من بياناتك ✓', 'info');
      }
    }
    syncState = 'ok';
    lastSyncError = '';
