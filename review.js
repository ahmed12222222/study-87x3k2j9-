    ...
    listEl.innerHTML = item.questions.map(q => `
      <div class="qa-row">
        ...
        <div class="qa-row-text">
          <div class="qa-question">${q.question ? escapeHtml(q.question) : '<span class="qa-empty-hint">(بدون نص سؤال — صورة بس)</span>'}</div>
...
function addQuestionToItem(){
  ...
  if(!question && !questionImageDraft){ toast('لازم تكتب سؤال أو تختار صورة على الأقل', 'error'); return; }
  if(!item.questions) item.questions = [];
  item.questions.push({ id: uid(), question, answer, image: questionImageDraft || null });
  persist();
  renderQuestionsList(item);
  renderAllTopics();
  renderTodayLists();
...
function renderQuizStep(){
  ...
  body.innerHTML = `
    ...
    ${q.question ? `<div class="quiz-question">${escapeHtml(q.question)}</div>` : (q.image ? '' : `<div class="quiz-question quiz-question-empty">(سؤال بدون نص)</div>`)}
    ${quizState.revealed ? `
